...
class ChromeRemoteDebugInterface(ChromeRemoteDebugInterface_base):
    ...
    def __init__(self,
        ...
        if visible_size:
            ...
            assert all(
                [isinstance(val, int) for val in visible_size]), "visible_size must be a 2-tuple containing 2 integers"
    ...
    def set_cookie(self, cookie):
        ...
        assert isinstance(cookie,
                          http.cookiejar.Cookie), 'The value passed to `set_cookie` must be an instance of http.cookiejar.Cookie().' + \
    ...
    def handle_page_location_changed(self, timeout=None):
        ...
        resp = self.transport.recv_filtered(
            filter_funcs.network_response_recieved_for_url(last_message['params']['documentURL'],
                                                           last_message['params']['frameId']), tab_key=self.tab_id)
        resp = resp['params']
        ...
        try:
            while 1:
                if time.time() - start_time > max_wait_timeout:
                    self.log.debug("Page was not idle after waiting %s seconds. Giving up and extracting content now.",
                                   max_wait_timeout)
                    break
    ...
    def blocking_navigate(self, url, timeout=DEFAULT_TIMEOUT_SECS):
        ...
        try:
            ...
            self.transport.recv_filtered(filter_funcs.check_frame_load_command("Page.frameStartedLoading"),
                                         tab_key=self.tab_id, timeout=timeout)
            self.log.debug("Waiting for frameStoppedLoading response.")
            self.transport.recv_filtered(filter_funcs.check_frame_load_command("Page.frameStoppedLoading"),
                                         tab_key=self.tab_id, timeout=timeout)
            ...
            resp = self.transport.recv_filtered(
                filter_funcs.network_response_recieved_for_url(url=None, expected_id=expected_id), tab_key=self.tab_id,
                timeout=timeout)
