...
class ChromeExecutionManager():
    ...
    def __init__(self,
        ...
        if port in ACTIVE_PORTS:
            raise cr_exceptions.ReusedPortError(
                "Attempting to start chromium using a already-in-use debug port (%s, %s)!" % (port, ACTIVE_PORTS))
    ...
    def __connect_to_tab(self, tab_key):
        ...
        if not 'webSocketDebuggerUrl' in cr_tab_meta:
            raise cr_exceptions.ChromeConnectFailure(
                "Tab %s has no 'webSocketDebuggerUrl' (%s)" % (tab_key, self.tablist))
    ...
    def recv_filtered(self, keycheck, tab_key, timeout=30, message=None):
        ...
        while 1:
            ...
            if time.time() > timeout_at:
                if message:
                    raise cr_exceptions.ChromeResponseNotReceived(
                        "Failed to receive response in recv_filtered() (%s)" % message)
                else:
